  ...
  const formatValue = (value) => {
    ...
    if (typeof value === 'string') {
      ...
      if (!isNaN(numberValue)) {
          ...
          return value;  // 如果转换失败，返回原始字符串值
      }
    }
    if (typeof value === 'number') {
      return value.toFixed(2);
    }
  return value;  // 如果是其他类型，直接返回
  }
 ...
 return (
  ...
  <div className="loadingContainer">
   ...
   <DeckGL
    ...
    getTooltip={({ object }) => {
      if (!object) return;
      const rank = Math.round(object.properties?.rank);
      const 总长度 = formatValue(object.properties?.["总长度"]);
      const 挑战性 = formatValue(object.properties?.["挑战性"]);
      const 服务设施完善程度 = formatValue(object.properties?.["服务设施完善程度"]);
      const 气候状况 = formatValue(object.properties?.["气候状况"]);
      const 热门程度 = formatValue(object.properties?.["热门程度"]);
      const 综合评分 = formatValue(object.properties?.["综合评分"]);
      return `排名：${rank}
          总长度：${总长度}米
          挑战性：${挑战性}
          服务设施完善程度：${服务设施完善程度}
