...
const IconSlider1 = ({ setlandscape_compact, setLandscapeCompact,...props }) => {
    ...
    return (
      ...
      <div className="icon-wrapper">
        ...
        <Slider {...props} onChange={handleSliderChange1} value={value} defaultValue={2} />
  ...
  const IconSlider2 = ({ setlandscape_loose,setLandscapeLoose, ...props }) => {
    const { max, min } = props;
    const [value, setValue] = useState( setlandscape_loose)
    const mid = Number(((max - min) / 2).toFixed(5))
    const preColorCls = value >= mid ? '' : 'icon-wrapper-active'
    const nextColorCls = value >= mid ? 'icon-wrapper-active' : ''
    const textStyle = {
    ...
    return (
      <div className='iconslider-container extra-bottom-margin'>
      <span style={textStyle}>松散度</span>
      <div className="icon-wrapper">
        <FrownOutlined className={preColorCls} />
        <Slider {...props} onChange={handleSliderChange2} value={value} defaultValue={2} />
        <SmileOutlined className={nextColorCls} />
      </div>
      </div>
    )
  ...
  const IconSlider3 = ({ setdemand_compact,setDemandCompact, ...props }) => {
    const { max, min } = props;
    const [value, setValue] = useState(setdemand_compact)
    const mid = Number(((max - min) / 2).toFixed(5))
    const preColorCls = value >= mid ? '' : 'icon-wrapper-active'
    const nextColorCls = value >= mid ? 'icon-wrapper-active' : ''
    const textStyle = {
    ...
    return (
      ...
      <div className="icon-wrapper">
        <FrownOutlined className={preColorCls} />
        <Slider {...props} onChange={handleSliderChange3} value={value} defaultValue={2} />
        <SmileOutlined className={nextColorCls} />
      </div>
      </div>
    )
  ...
  const IconSlider4 = ({ setdemand_loose,setDemandLoose, ...props }) => {
    const { max, min } = props;
    const [value, setValue] = useState(setdemand_loose)
    const mid = Number(((max - min) / 2).toFixed(5))
    const preColorCls = value >= mid ? '' : 'icon-wrapper-active'
    const nextColorCls = value >= mid ? 'icon-wrapper-active' : ''
    const textStyle = {
    ...
    return (
      ...
      <div className="icon-wrapper">
        <FrownOutlined className={preColorCls} />
        <Slider {...props} onChange={handleSliderChange4} value={value} defaultValue={2} />
        <SmileOutlined className={nextColorCls} />
      </div>
      </div>
    )
